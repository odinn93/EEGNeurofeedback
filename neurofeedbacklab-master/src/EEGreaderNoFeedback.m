...
function EEGreaderNoFeedback(base, varargin)
disp('beginning of reader')
idr = labindex;
nfblab_optionsNoFeedback;
try
        ...
        if dataBufferPointer > chunkSize
            ...
            if ~isempty(lastChunkTime) && warnsrate
                ...
                if abs(srateHardware-sRateEstimated) > 0.1*srateHardware
                    fprintf('Warning: estimated heart rate %d Hz compared to %d Hz set in nfblab_optionsNoFeedback.m\n', round(sRateEstimated), round(srateHardware));
                end
            end
            lastChunkTime = toc;
