% *************************************
%
% MAIN SETTNGS BELOW FOR EEGreaderNoFeedback
%
% *************************************
%runmode = 'slave';
...
% LSL parameters
% --------------
p = fileparts(which('nfblab_optionsNoFeedback.m'));
...
% -----------------------------------------
baselineSessionDuration = 55; % duration of baseline in second (the baseline is used
                              % to train the artifact removal ASR function)
sessionDuration = 60*1.45; % regular (trial) sessions
...
BCILABpath     = 'C:\Users\VR\OneDrive\Dokument\MATLAB\EEGFeedback\neurofeedbacklab-master\BCILAB'; 
if ~isdeployed
    if ~exist(BCILABpath, 'dir')
        ...
        if ~exist(BCILABpath, 'dir')
            BCILABpath = fullfile(p, '..', '..', 'BCILAB');
            if ~exist(BCILABpath, 'dir')
               error('Cannot find BCILAB - set path manually in file nfblab_optionsNoFeedback.m');
            end
        end
    end
