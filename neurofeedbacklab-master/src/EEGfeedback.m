...
spinTime = 60;      %Duration of spinning animation
...
while 1
    ...
    %Flip to the screen
     ...
     % Checks to see if the other program is ready to send. It does mean
     % that some signals can be "lost" if the two programs don't sync. 
     % An alternative is increasing the recording frequency in
     % nfblab_options (reduce pauseSecond) but that can negatively impact
     % performance.
     if labProbe(1,1)
         feedback = labReceive(1,1);  % Receiver ID, tag for indentif.
     else
         feedback = oldfeedback; % If it doesn't detect new data, it uses
     end                         % the old one
     angles = 0.05+feedback*1.5; % Adjust these values as you wish
