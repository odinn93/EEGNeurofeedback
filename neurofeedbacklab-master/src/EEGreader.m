% The following is a modification of "nfblab_process.m" from the 
% Neurofeedbacklab software by Arnaud Delorme.
% (See https://github.com/arnodelorme/neurofeedbacklab)
%
% Attempts have been made to remove redundant code that isn't relevant to
% this project but please note that there might still be some remnants of
% overlooked code that dont serve any purpose.
%
% The following instructions are copied from "nfblab_process.m"
% NOTE THAT THIS CODE IS STILL DEPENDENT ON "nfblab_options.m"
%
% Usage:
...
function EEGreader(base, varargin)
idr = labindex;
nfblab_options;
try
...
if asrFlag == false
    ...
    disp('Note: default ASR introduces a delay - in our experience 1/4 second)');
end
if ~exist('runmode')
    TCPIP = false;
    %id = labindex;      % Parallel computing ID
    %s = input('Do you want to run a baseline now (y/n)?', 's');
    runmode = 'trial';
    %labSend(1,id+1,1);  % Data sent, receiver ID, tag for indentif.
end
...
while 1
    ...
    %Because labReceive pauses a program until it receives data from
    %labSend, we need to regularly send the old value in order to ensure
    %that EEGfeedback runs continuously
    ...
    if strcmpi(currentMode, 'run')
        ...
        labSend(feedbackVal,2,1);  % Data sent, receiver ID, tag for indentif.
    end
end
end